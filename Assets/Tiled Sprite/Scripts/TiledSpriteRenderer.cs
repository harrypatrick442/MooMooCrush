 ...
 *
 *
 * UPDATE February 2017 (version 1.2.2)
 * 
 * Added support for dynamic atlases packed by built-in Sprite Packer. Basically, the texture
 * at run time is not the same as in the editor. Only "Full Rect" mesh type is supported.
 * 
...
#region methods	
	...
	protected virtual void Start()
	{
		if (regenerateMesh) {
            ...
            if ((m_sprite != null) && (m_sprite.texture != null) && m_sprite.packed && Application.isPlaying) {
                // Automatically packed atlases with the built-in Sprite Packer need a UV update.
                RecalculateTexUV();
            }		
	...
	void UpdateMesh(bool _forceColorUpdate)
	{
        ...
        bool edgesOutOfBounds;
		
        leftTopTileSize = CalculateFragmentsAndEdges(out tileEdgeCountX, out tileEdgeCountY, out edgesOutOfBounds);
        if (edgesOutOfBounds) {
        ...
        tileEdgeCount = new Vector2(tileEdgeCountX, tileEdgeCountY);
        tileFillX = leftTopTileSize.x;
        tileFillY = leftTopTileSize.y;
    ...
    Vector2 CalculateFragmentsAndEdges(out int _edgeCountX, out int _edgeCountY, out bool _outOfBounds)
    {
        ...
        if (m_sprite != null)
        {
            Vector2 tileSize = m_sprite.bounds.size;
            float halfPixelSize = tileSize.x / m_sprite.rect.width * 0.5f;
        ...
        _edgeCountX = tileEdgeCountX;
        _edgeCountY = tileEdgeCountY;

        return new Vector2(tileFillX, tileFillY);
    }

    void RecalculateTexUV()
    {
        // Calculate edge count and left top fragment.

        int edgeCountX = (int) tileEdgeCount.x,
            edgeCountY = (int) tileEdgeCount.y;
        bool outOfBoundsError = false;

        if ((edgeCountX < 0) || (leftTopTileSize.x < 0)) {
            leftTopTileSize = CalculateFragmentsAndEdges(out edgeCountX, out edgeCountY, out outOfBoundsError);
            tileEdgeCount = new Vector2(edgeCountX, edgeCountY);
        }

        // Prepare all possible values for coordinates.
		...
		if (m_sprite != null) {
            ...
            Vector2 u = GetTexCoords1D(tileSize.x, m_size.x, leftTopTileSize.x, edgeCountX - 1, new Vector2(texRect.xMin, texRect.xMax));
			Vector2 v = GetTexCoords1D(tileSize.y, m_size.y, leftTopTileSize.y, edgeCountY - 1, new Vector2(texRect.yMin, texRect.yMax));
			uvMargins = new Vector4(u.x, u.y, v.x, v.y);
        ...
        // ... first horizontal tile
        uv[index] = new Vector2(uvMargins[0], uvMargins[2]);
        uv[index + 1] = new Vector2(uvMargins[0], uv01[3]);
        uv[index + 2] = new Vector2(uv01[1], uv01[3]);
        uv[index + 3] = new Vector2(uv01[1], uvMargins[2]);
        index += 4;

        // ... middle tiles
        for (int j = edgeCountX - 3; j > 0; --j)
        {
            uv[index] = new Vector2(uv01[0], uvMargins[2]);
            uv[index + 1] = new Vector2(uv01[0], uv01[3]);
            uv[index + 2] = new Vector2(uv01[1], uv01[3]);
            uv[index + 3] = new Vector2(uv01[1], uvMargins[2]);
            index += 4;
        }
        ...
        if (edgeCountX > 2)
        {
            uv[index] = new Vector2(uv01[0], uvMargins[2]);
            uv[index + 1] = new Vector2(uv01[0], uv01[3]);
            uv[index + 2] = new Vector2(uvMargins[1], uv01[3]);
            uv[index + 3] = new Vector2(uvMargins[1], uvMargins[2]);
            index += 4;
        }

        // middle
        for (int i = edgeCountY - 3; i > 0; --i)
        {
            // ... first horizontal tile
            uv[index] = new Vector2(uvMargins[0], uv01[2]);
            uv[index + 1] = new Vector2(uvMargins[0], uv01[3]);
            uv[index + 2] = new Vector2(uv01[1], uv01[3]);
            uv[index + 3] = new Vector2(uv01[1], uv01[2]);
            index += 4;

            // ... middle tiles
            for (int j = edgeCountX - 3; j > 0; --j)
            {
                uv[index] = new Vector2(uv01[0], uv01[2]);
                uv[index + 1] = new Vector2(uv01[0], uv01[3]);
                uv[index + 2] = new Vector2(uv01[1], uv01[3]);
                uv[index + 3] = new Vector2(uv01[1], uv01[2]);
                index += 4;
            ...
            if (edgeCountX > 2)
            {
                ...
                uv[index + 1] = new Vector2(uv01[0], uv01[3]);
                uv[index + 2] = new Vector2(uvMargins[1], uv01[3]);
                uv[index + 3] = new Vector2(uvMargins[1], uv01[2]);
                index += 4;
            }
        }
        ...
        if (edgeCountY > 2)
        {
            // ... first horizontal tile
            uv[index] = new Vector2(uvMargins[0], uv01[2]);
            uv[index + 1] = new Vector2(uvMargins[0], uvMargins[3]);
            uv[index + 2] = new Vector2(uv01[1], uvMargins[3]);
            uv[index + 3] = new Vector2(uv01[1], uv01[2]);
            index += 4;
            ...
            {
                uv[index] = new Vector2(uv01[0], uv01[2]);
                uv[index + 1] = new Vector2(uv01[0], uvMargins[3]);
                uv[index + 2] = new Vector2(uv01[1], uvMargins[3]);
                uv[index + 3] = new Vector2(uv01[1], uv01[2]);
                index += 4;
            }
            ...
            if (edgeCountX > 2)
            {
                uv[index] = new Vector2(uv01[0], uv01[2]);
                uv[index + 1] = new Vector2(uv01[0], uvMargins[3]);
                uv[index + 2] = new Vector2(uvMargins[1], uvMargins[3]);
                uv[index + 3] = new Vector2(uvMargins[1], uv01[2]);
                index += 4;
            }
        }

        // Handle flipping and commit.
...
#region fields
    ...
    [SerializeField]
    [HideInInspector]
    Vector2 leftTopTileSize = new Vector2(-1, -1); // Left, top edge tile size.

    [SerializeField]
    [HideInInspector]
    Vector2 tileEdgeCount = new Vector2(-1, -1);
